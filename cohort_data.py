...
def unique_houses(filename):
    ...
    """
    my_file = open(filename)  #importing file
    my_house = []  #new list to contain houses
    for line in my_file:  #looping over each line in filename
        entry = line.split("|")  #splitting by |, returns a list
        if (entry[2] != ""):  #removes empty entries (aka no houses)
            my_house.append(entry[2])  #appends houses into empty list my_house
    houses = set(my_house)    #makes unique (aka removes duplicate entries)
...
def sort_by_cohort(filename):
    ...
    for line in my_file:
        first_name, last_name, the_house, my_advisor, my_cohort = line.split("|")
        my_cohort_rev = my_cohort.rstrip()
        # print(my_cohort)
        if my_cohort_rev == 'Fall 2015':
            fall_15.append(f'{first_name} {last_name}')
        if my_cohort_rev == 'Summer 2016':
            summer_16.append(f'{first_name} {last_name}')
        if my_cohort_rev == 'Spring 2016':
            spring_16.append(f'{first_name} {last_name}')
        if my_cohort_rev == 'Winter 2016':
...
def hogwarts_by_house(filename):
    ...
    for line in my_file:
        ...
        if the_house == "Dumbledore's Army":
            dumbledores_army.append(last_name)
        if the_house == 'Gryffindor':
            gryffindor.append(last_name)
        if the_house == 'Hufflepuff':
            hufflepuff.append(last_name)
        if the_house == 'Ravenclaw':
            ravenclaw.append(last_name)
        if the_house == 'Slytherin':
    ...
    # Code goes here
    all_hogwarts = [dumbledores_army.sort(), gryffindor, hufflepuff, ravenclaw, slytherin, ghosts, instructors]
    return all_hogwarts
